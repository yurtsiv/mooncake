...
evaluate (AST.FunctionCall name callArgs) s = do
  (res, s) <- evaluate (AST.Identifier name) s
  case res of
    Function argNames body ->
      if (length callArgs) /= (length argNames)
        then Left $ "Wrong number of arguments provided for " ++ name
        else
          let evaluatedArgs = evaluate (AST.List callArgs) s
           in case evaluatedArgs of
                Right (List evalArgs, _) ->
                  let funcScope = mergeScopes s (Map.fromList $ (zip argNames evalArgs))
